...
/**
 * Trust-boundary regression tests for Clinic Doctor.
 *
...
describe('Clinic Doctor — window security configuration', () => {
  const main = read('src/main/index.ts');
  ...
  it('serves renderer assets from a privileged custom scheme, not file://', () => {
    ...
    expect(APP_CONFIG.assetProtocolScheme).toBe('clinic-doctor-app');
    expect(read('src/shared/app-config.ts')).toContain("'clinic-doctor-app'");
  });
...
describe('Clinic Doctor — Electron fuses', () => {
...
describe('Clinic Doctor — IPC contract', () => {
...
describe('Clinic Doctor — application identity', () => {
  it('uses namespaces distinct from the sibling desktop application', () => {
    ...
    expect(APP_CONFIG.appId).toBe('eg.clinic.doctor.desktop');
    expect(APP_CONFIG.userDataDirectory).toBe('clinic-doctor');
    expect(APP_CONFIG.protocolScheme).toBe('clinic-doctor');
    expect(APP_CONFIG.assetProtocolScheme).toBe('clinic-doctor-app');
    expect(APP_CONFIG.deviceCredentialNamespace).toBe('eg.clinic.doctor.device');
    expect(APP_CONFIG.encryptedDatabaseNamespace).toBe('doctor.encrypted.v1');
    expect(APP_CONFIG.updateChannel).toBe('doctor-stable');

    // Nothing may carry the sibling's identity.
    const serialized = JSON.stringify(APP_CONFIG);
    expect(serialized).not.toContain('pharmacy');
  });
