...
/**
 * Clinic Doctor packaged API origin policy.
 *
 * These hand the resolver hostile URLs. A substring assertion cannot tell a
 * real allowlist from a comment that mentions one.
 */
describe('Clinic Doctor — packaged API exact-origin allowlist', () => {
  const APPROVED = 'https://api.example.com';
  const SIBLING = 'https://pharmacy.example.com';
  const PACKAGED = true;
  ...
  it('A. accepts the approved exact HTTPS origin', () => {
    ...
    expect(resolve(`${APPROVED}/`, PACKAGED)).toBe(APPROVED);
    expect(resolve('https://api.example.com:443', PACKAGED)).toBe(APPROVED);
  });
  ...
  it('C. denies HTTP even when the hostname is the approved host', () => {
    expect(() => resolve('http://api.example.com', PACKAGED)).toThrow('INSECURE_TRANSPORT');
  });
  ...
  it('F. denies a lookalike hostname', () => {
    expect(() => resolve('https://api.example.com.evil.test', PACKAGED)).toThrow('ORIGIN_REFUSED');
  });

  it('G. denies a credentialed URL even when the host would match', () => {
    expect(() => resolve('https://user:pass@api.example.com', PACKAGED)).toThrow('ORIGIN_REFUSED');
  });

  it('H. denies a wrong explicit port', () => {
    expect(() => resolve('https://api.example.com:444', PACKAGED)).toThrow('ORIGIN_REFUSED');
  });
  ...
  it('rejects unexpected path, query, hash, and malformed values', () => {
    expect(() => resolve('https://api.example.com/v1', PACKAGED)).toThrow('ORIGIN_REFUSED');
    expect(() => resolve('https://api.example.com?x=1', PACKAGED)).toThrow('ORIGIN_REFUSED');
    expect(() => resolve('https://api.example.com#frag', PACKAGED)).toThrow('ORIGIN_REFUSED');
    expect(() => resolve('not a url', PACKAGED)).toThrow('ORIGIN_REFUSED');
    expect(() => resolve('https://api.example.com', PACKAGED)).not.toThrow();
  });
  ...
  it('does not let runtime env redefine the baked packaged allowlist', () => {
    ...
    process.env['CLINIC_DOCTOR_PACKAGED_API_ALLOWED_ORIGINS'] = 'https://evil.example';
  ...
  it('treats an empty build-time env as an empty allowlist and rejects invalid bake entries', () => {
    ...
    expect(parsePackagedAllowlistEntries('https://api.example.com')).toEqual(['https://api.example.com']);
    expect(() => parsePackagedAllowlistEntries('http://api.example.com')).toThrow('PACKAGED_ALLOWLIST_INVALID');
    expect(() => parsePackagedAllowlistEntries('https://user:pass@api.example.com')).toThrow(
