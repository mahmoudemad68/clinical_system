...
describe('doctor evidence handles', () => {
  const roots: string[] = [];
  ...
  it('rejects an unknown handle without leaking a path', () => {
    const store = new DoctorEvidenceHandleStore();
    try {
      store.readForUpload('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa');
      throw new Error('expected unknown handle to fail');
    } catch (error) {
      expect(error).toBeInstanceOf(EvidenceFileError);
      expect((error as EvidenceFileError).code).toBe('FILE_MISSING');
      expect(String(error)).not.toContain(CANARY_PATH);
      expect(String(error)).not.toContain('/tmp/');
    }
  });

  it('rejects a handle from a previous cleared session', () => {
    const dir = scratch();
    const file = path.join(dir, 'registration.pdf');
    writeFileSync(file, MIN_PDF);
    const store = new DoctorEvidenceHandleStore();
  ...
  it('does not include the local path on inspect or upload failures', () => {
    try {
      inspectCandidateFile(CANARY_PATH);
    } catch (error) {
      expect(JSON.stringify(error)).not.toContain(CANARY_PATH);
      expect(String(error)).not.toContain(CANARY_PATH);
      expect(String(error)).not.toContain(CANARY_URL);
    }
  });
});
