...
const { fetchMock, persistMock, loadMock, clearMock, appState, allowlistState } = vi.hoisted(() => ({
  ...
  allowlistState: { origins: ['https://api.example.com'] as string[] },
}));
...
describe('Clinic Doctor — platform gateway origin and refresh', () => {
  ...
  it('C. HTTP approved hostname is refused before net.fetch when packaged', async () => {
    await expectDeniedBeforeFetch('http://api.example.com');
  });
  ...
  it('F. lookalike hostname is refused before net.fetch', async () => {
    await expectDeniedBeforeFetch('https://api.example.com.evil.test');
  });
  ...
  it('H. wrong explicit port is refused before net.fetch', async () => {
    await expectDeniedBeforeFetch('https://api.example.com:444');
  });
...
describe('Clinic Doctor — cookieless device transport', () => {
  const previousApi = process.env['CLINIC_API_BASE_URL'];
  ...
  it('uses cookieless credentials for health, login, MFA, refresh, and authenticated Core calls', async () => {
    ...
    expect(String(fetchMock.mock.calls[2]?.[0])).toContain('/auth/mfa/');
    expectCookielessDeviceFetch(2);
    expect(String(fetchMock.mock.calls[3]?.[0])).toContain('/api/v1/me');
    expectCookielessDeviceFetch(3, { bearer: true });
    expect(String(fetchMock.mock.calls[4]?.[0])).toContain('/api/v1/me/capabilities');
  ...
  it('refreshes with the refresh token body and no cookie session', async () => {
    ...
    expect(String(fetchMock.mock.calls[1]?.[0])).toContain('/api/v1/auth/token/refresh');
