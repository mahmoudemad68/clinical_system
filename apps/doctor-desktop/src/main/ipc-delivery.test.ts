...
import { doctorOnboardResponseSchema } from '@clinic/desktop-bridge-contracts';
...
describe('IPC delivery tickets', () => {
  ...
  it('reuses the same key for open and submit after a caller-visible timeout', async () => {
    ...
    await timeoutThenRetry('doctor.verification.open', { intent: 'doctor.verification.open' });
    await timeoutThenRetry('doctor.verification.submit', { caseVersion: 1, profileVersion: 1 });
  });

  it('does not retire the key when the resolved value fails the IPC response contract', async () => {
    const store = new IntentKeyStore();
    const fingerprint = store.fingerprint({ nationalId: 'A' });
