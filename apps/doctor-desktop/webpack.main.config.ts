 ...
 *
 * `CLINIC_DOCTOR_PACKAGED_API_ALLOWED_ORIGINS` is a build-time input only.
 ...
 * fails closed when the packaged app runs. This env name is Doctor-specific
 * so a Pharmacy bake cannot accidentally become this application's trust
 * list.
 */
const packagedApiAllowedOrigins = parsePackagedAllowlistEntries(
  process.env['CLINIC_DOCTOR_PACKAGED_API_ALLOWED_ORIGINS'] ?? '',
);
