...
/**
 * Two OS processes, two PostgreSQL sessions, one append each after a file barrier.
 */
final class ConcurrentAuditAppend
{
    /**
    ...
    {
        $dir = sys_get_temp_dir().'/clinic-audit-race-'.bin2hex(random_bytes(8));
        if (! mkdir($dir, 0700) && ! is_dir($dir)) {
            throw new RuntimeException('Could not create audit race barrier directory.');
        }
    ...
    {
        $worker = base_path('tests/Support/bin/audit-append-worker.php');
        ...
        $process = proc_open($command, $spec, $pipes, base_path(), self::workerEnv());
        if (! is_resource($process)) {
            throw new RuntimeException('proc_open failed for audit append worker.');
        }
    ...
    private static function finish(array $proc, float $timeoutSeconds): array
    {
        ...
        $decoded['stderr'] = substr($stderr, 0, 500);
    ...
    private static function workerEnv(): array
    {
        ...
        foreach (['DB_HOST', 'DB_PORT', 'DB_DATABASE', 'DB_AUDIT_USERNAME', 'DB_AUDIT_PASSWORD'] as $key) {
            $value = $_ENV[$key] ?? $_SERVER[$key] ?? null;
            if (is_string($value) && $value !== '' && (! isset($env[$key]) || $env[$key] === '')) {
                $env[$key] = $value;
            }
        }
    ...
    private static function waitForFile(string $path, float $timeoutSeconds): void
    {
        ...
        while (! is_file($path)) {
            if (microtime(true) > $deadline) {
                throw new RuntimeException('Audit append worker did not reach the start barrier.');
            }
            usleep(1_000);
        }
    }
