...
it('keeps worker and reporter off clinic tables', function () {
...
it('lets clinic_app mutate clinic tables and backup only select', function () {
    ...
    $backupSelect = DB::selectOne("SELECT has_table_privilege('clinic_backup', 'clinic_locations', 'SELECT') AS allowed");
    $backupInsert = DB::selectOne("SELECT has_table_privilege('clinic_backup', 'clinic_locations', 'INSERT') AS allowed");
