...
import {
  ...
  requirementLabel,
  type PharmacyRequirement,
  type PharmacyRequirementCode,
...
const SCANNING_STATES: ReadonlySet<PharmacyUploadStatus['state']> = new Set([
  'requested',
  'uploading',
  'quarantined',
  'validating',
...
type SlotState = {
  evidence: Extract<PharmacyEvidenceSelectResponse, { selected: true }> | null;
  uploadId: string | null;
  pollTimedOut: boolean;
  pollGeneration: number;
};

function emptySlots(): Record<PharmacyRequirementCode, SlotState> {
  return {
    pharmacy_facility_license: { evidence: null, uploadId: null, pollTimedOut: false, pollGeneration: 0 },
    commercial_register: { evidence: null, uploadId: null, pollTimedOut: false, pollGeneration: 0 },
    responsible_pharmacist_license: { evidence: null, uploadId: null, pollTimedOut: false, pollGeneration: 0 },
  };
}
...
function uploadStatusCopy(locale: Locale, state: PharmacyUploadStatus['state']): string {
  const t = pharmacyStrings[locale].verification;
  if (state === 'requested') {
    return t.requested;
  }
  if (state === 'uploading') {
...
function documentForRequirement(
  status: PharmacyVerificationStatus | undefined,
  code: PharmacyRequirementCode,
) {
...
export function VerificationWorkspace({
  ...
  locale: Locale;
  organization: PharmacyOrganizationView;
}) {
  ...
  const statusQuery = useQuery({
    queryKey: ['pharmacy', 'verification', locale],
    queryFn: async () => {
      const result = await window.clinic.pharmacy.verificationStatus();
      if (!result.ok) {
  ...
  const evidenceReady = requiredDocumentsReady(status?.documents ?? [], PHARMACY_REQUIREMENTS);
  const canResubmit =
    status?.organizationVerificationStatus === 'changes_requested' ||
    status?.organizationVerificationStatus === 'rejected';
  const pendingReview = status?.organizationVerificationStatus === 'pending_review';
  const approved = status?.organizationVerificationStatus === 'approved';
  ...
  useEffect(() => {
    if (approved) {
      void client.invalidateQueries({ queryKey: ['pharmacy', 'organization'] });
    }
  }, [approved, client]);

  async function refreshAuthoritative(): Promise<void> {
    await statusQuery.refetch();
    await client.invalidateQueries({ queryKey: ['pharmacy', 'organization'] });
  }
  ...
  async function openCase(): Promise<void> {
    ...
    setMessage(null);
    setSlots(emptySlots());
    const result = await window.clinic.pharmacy.openVerificationCase();
    setBusy(false);
    if (!result.ok) {
      setMessage(pharmacyErrorMessage(locale, result.error.code));
  ...
  async function chooseFile(code: PharmacyRequirementCode): Promise<void> {
    setMessage(null);
    const result = await window.clinic.pharmacy.selectEvidence({ requirementCode: code });
    if (!result.ok) {
      setMessage(pharmacyErrorMessage(locale, result.error.code));
      return;
    }
  ...
  async function upload(code: PharmacyRequirementCode): Promise<void> {
    ...
    setBusy(true);
    setMessage(null);
    const result = await window.clinic.pharmacy.uploadEvidence({
  ...
  async function submit(): Promise<void> {
    ...
    setBusy(true);
    setMessage(null);
    const result = await window.clinic.pharmacy.submitVerification({
  ...
  return (
    <Stack spacing={2} data-testid="verification-workspace">
      ...
      <Typography>{t.verification.noOperationalNav}</Typography>
      <Typography>
        {organization.publicName} — {t.status.organization}: {status?.organizationVerificationStatus ?? organization.verificationStatus}
      </Typography>
      <Typography>
        {t.status.lifecycle}: {status?.organizationStatus ?? organization.status}
      </Typography>
      <Typography>
        {t.status.branch}: {organization.initialBranch.publicName} / {organization.initialBranch.status}
      </Typography>
      <Typography>
        {t.status.membership}: {organization.membership.status}
      </Typography>
      {reasonCopy ? (
        <Typography data-testid="verification-reason">
          {t.status.reason}: {reasonCopy}
        </Typography>
      ) : null}
      ...
      {statusQuery.isSuccess && !status?.caseId && !pendingReview && !approved ? (
      ...
      {draftCase
        ? PHARMACY_REQUIREMENTS.map((requirement) => (
            <RequirementSlot
              ...
              onClear={() => {
                ...
                void window.clinic.pharmacy.clearEvidence(handleId).then(() => {
                  setSlots((current) => ({
                    ...current,
...
function RequirementSlot({
  ...
  locale: Locale;
  requirement: PharmacyRequirement;
  slot: SlotState;
  status: PharmacyVerificationStatus | undefined;
  busy: boolean;
  onChoose: () => void;
  ...
  const uploadQuery = useQuery({
    queryKey: ['pharmacy', 'upload', requirement.code, slot.uploadId, locale, slot.pollGeneration],
    enabled: slot.uploadId !== null,
    queryFn: async () => {
      if (slot.uploadId === null) {
        throw new Error('NOT_FOUND');
      }
      const result = await window.clinic.pharmacy.uploadStatus(slot.uploadId);
      if (!result.ok) {
        throw new Error(result.error.code);
      }
      return result.value;
