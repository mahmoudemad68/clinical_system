...
export function TeamSection({ locale, branchId }: { locale: Locale; branchId: string }) {
  const t = pharmacyStrings[locale];
  const [phone, setPhone] = useState('');
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<string | null>(null);
  const [invitation, setInvitation] = useState<PharmacyBranchInviteOperatorResponse | null>(null);
  const [pendingRevoke, setPendingRevoke] = useState<PharmacyBranchMembershipView | null>(null);
  const alertRef = useRef<HTMLDivElement>(null);

  const membershipsQuery = useQuery({
    queryKey: ['pharmacy', 'memberships', branchId, locale],
    queryFn: async () => {
      const result = await window.clinic.pharmacy.listMemberships({ branchId });
      if (!result.ok) {
  ...
  async function invite(): Promise<void> {
    ...
    const result = await window.clinic.pharmacy.inviteOperator({ branchId, phone: submittedPhone });
    setPhone('');
    setBusy(false);
    if (!result.ok) {
  ...
  async function revoke(membership: PharmacyBranchMembershipView): Promise<void> {
    setBusy(true);
    setMessage(null);
    const result = await window.clinic.pharmacy.revokeMembership({
      branchId,
  ...
  return (
    <Stack spacing={3} data-testid="branch-team">
      <Stack
        spacing={2}
        component="form"
        data-testid="operator-invite-form"
        onSubmit={(event) => {
      ...
      >
        ...
        <Typography data-testid="fixed-role">{t.practice.fixedRole}</Typography>
        {message ? (
        ...
        <TextField
          id="operator-invite-phone"
          label={t.practice.phone}
          value={phone}
          type="tel"
          required
          autoComplete="off"
          slotProps={{ htmlInput: { 'data-testid': 'operator-invite-phone' } }}
