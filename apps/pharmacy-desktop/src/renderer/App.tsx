...
function VerificationWorkspace({
  ...
  const [message, setMessage] = useState<string | null>(null);
  const [evidence, setEvidence] = useState<Extract<PharmacyEvidenceSelectResponse, { selected: true }> | null>(null);
  ...
  const uploadQuery = useQuery({
    queryKey: ['pharmacy', 'upload', uploadId, locale],
    enabled: uploadId !== null,
    queryFn: async () => {
      if (uploadId === null) {
        throw new Error('NOT_FOUND');
      }
      const result = await window.clinic.pharmacy.uploadStatus(uploadId);
      if (!result.ok) {
        throw new Error(result.error.code);
      }
      return result.value;
    ...
    refetchInterval: (query) => {
      ...
      if (state === 'quarantined' || state === 'validating' || state === 'scanning' || state === 'requested' || state === 'uploading') {
        return 2_000;
      }
      return false;
    },
  ...
  async function chooseFile(): Promise<void> {
    setMessage(null);
    const result = await window.clinic.pharmacy.selectEvidence();
    if (!result.ok) {
      setMessage(errorMessage(locale, result.error.code));
      return;
    }
  ...
  return (
    <Stack spacing={2} data-testid="verification-workspace">
      ...
      {status?.reasonCode ? (
        <Typography>
          {t.status.reason}: {status.reasonCode}
        </Typography>
      ) : null}
      ...
      {status?.caseStatus === 'draft' ? (
        <Stack spacing={1}>
          ...
          {evidence ? (
            <Typography>
              {t.verification.selectedFile}: {evidence.displayName} ({t.verification.size} {String(evidence.sizeBytes)}, {t.verification.type} {evidence.candidateMediaType})
            </Typography>
          ) : null}
          {evidence ? (
            <Button
              type="button"
              onClick={() => {
                void window.clinic.pharmacy.clearEvidence(evidence.handleId).then(() => setEvidence(null));
              }}
            >
      ...
      {status?.caseStatus === 'draft' && !evidenceReady ? (
      ...
      {message ? (
        <Alert ref={alertRef} tabIndex={-1} severity="warning" role="alert">
          {message}
        </Alert>
      ) : null}
    </Stack>
