...
description: Implement or refactor Laravel 13 Core modules, application coordinators, policies, HTTP contracts, Eloquent adapters, and PHP tests for this clinic platform. Use for Core PHP work; domain and delivery specialists retain their invariants.
---
...
This skill owns Laravel implementation mechanics across `apps/core-api`: module layout, commands/queries/handlers, policies, coordinators, Eloquent repositories, HTTP resources, framework configuration, service-provider wiring, Laravel-side contract tests, and safe provider adapters.
...
Controllers handle transport input, authenticate, invoke one command/query handler, and map the stable response. They do not perform state transitions. Application handlers coordinate policies, transactions, domain objects, and ports. Infrastructure implements domain/application-owned ports; provider and Eloquent types never escape their adapter.
...
- Keep public endpoints under `/api/v1` and OpenAPI authoritative. Generate client types; do not hand-maintain a conflicting DTO contract.
...
- Use `brick/money` for financial value objects, framework/Symfony UUIDv7 support, `deptrac/deptrac`, Larastan/PHPStan, Pint, and the repository-standard Pest or PHPUnit runner. Add provider libraries only behind a narrow adapter and lock the resolved version.
...
- focused Pest/PHPUnit tests cover success, denial, stale version, same/different idempotency hash, duplicate delivery, rollback, and safe error mapping;
