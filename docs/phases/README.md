...
- Any deviation from `plan.md` or a phase invariant requires a short Architecture Decision Record (ADR), risk assessment, migration impact, and product/security approval.
...
| 06 | [Prescriptions, reminders, and printing](06_prescriptions_reminders_and_printing.md) | Structured prescriptions, immutable finalized versions, exposure/amendment audit, reminders, and print/export | 05; consumes a `MedicationReference` port whose production catalog adapter arrives in 10 |
...
| 16 | [AI platform, knowledge ingestion, and retrieval](16_ai_platform_knowledge_ingestion_and_retrieval.md) | Isolated AI service, versioned ingestion, hybrid retrieval, provider ports, and evaluation harness | 00, 07, 10 |
...
| 22 | [Security, privacy, and compliance validation](22_security_privacy_and_compliance_validation.md) | Cross-system threat closure, privacy/legal gates, penetration tests, and release security evidence | 00-21 |
...
- Acceptance sign-off by engineering, QA, product, security/privacy, and clinical/legal reviewers when the phase says they are required.

## Decisions that must close before implementation reaches the named gate

The source plan leaves several safety-significant choices open or internally ambiguous. The phase documents use the safest reversible default, but the accountable owners must record the final decision in an ADR before the related production gate:
...
| Minors, guardians, caregivers, deceased/incapacitated patients, and emergency break-glass | No proxy or break-glass access is enabled until legal/clinical policy defines scope, expiry, alerts, and audit. | Phase 01 scope gate; before any such use in production |
...
| Retention, correction, export, deletion, legal holds, and cross-border AI processing | Minimize and quarantine by default; do not send regulated data to an external model or set irreversible retention until Egyptian legal/privacy and clinical-record obligations are approved. | Phase 16 provider gate and Phase 22 release gate |
