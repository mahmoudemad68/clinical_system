...
- Persist every user-visible notification with **Laravel Database Notifications**. The database row is the inbox source of truth; push is delivery only.
- Send push through **Firebase** using **`kreait/laravel-firebase`** behind a narrow adapter. Do not add OneSignal, Pusher Beams, Expo push, or another primary push vendor.
