...
Keep domain code framework-independent. Route HTTP/jobs/events through Laravel application commands, persistence through PostgreSQL-owned repositories, doctor drafts through authorized Laravel/Inertia flows, patient reads through patient Inertia pages, and bytes through secure-file references. Return stable machine error codes with safe localized presentation; do not leak whether another patient's record exists.
