...
description: Implement or review this clinic project's clinical-care business rules for consultation access, encounters, medical records, prescriptions, labs, reports, referrals, and clinician draft synchronization. Use for Phases 04-07 or clinical constraints consumed by later AI/pharmacy work; not for generic Laravel/client tasks, pharmacy inventory/POS, file-scanner mechanics, or independent test/security assurance.
---

# Clinic Clinical Domain

Preserve clinical meaning and patient-safety invariants across framework, database, client, file, and AI implementations. Express behavior with conventional Laravel models, backed enums, policies, and cohesive module services.
...
- clinical module services, typed inputs/results, authorization context requirements, and Laravel event meaning.

The Laravel skill owns Nwidart module wiring, controllers, Form Requests, Resources, and service-provider bindings. PostgreSQL consistency owns migrations, constraints, locks, indexes, and transaction proofs. Doctor Inertia pages own clinician UI/sync presentation; patient Inertia pages own patient read surfaces. Secure files owns byte quarantine and signed delivery. AI skills own recommendation/retrieval mechanics. Test and security/privacy skills independently verify the implementation. Split clinical services by workflow; do not create one “clinical service” god class.
...
5. PostgreSQL is medical truth. Clinician drafts persist as server-owned transient work with explicit local-editing, pending, acknowledged, conflicted, failed, and purged states. They never silently overwrite newer server truth. Do not add an encrypted desktop SQLite cache unless a native client is explicitly required.
...
8. Prescription logic consumes a narrow `MedicationReferenceService`. Phase 06 must work with the interim implementation; it must not depend directly on the full Phase 10 catalog schema or a vendor API.
9. State transitions reject skipped, backward, stale, duplicate-with-different-payload, unauthorized, or context-expired service requests. Critical service methods are transactionally safe and idempotent.
...
12. Proxy/guardian, emergency break-glass, image diagnosis, medication alternatives, and other V1 exclusions remain disabled until their named product/clinical policy, security controls, and tests exist. Legal approval is not required.

## Conventional Laravel model

- Keep each module's models and services responsible for a clear workflow and transaction boundary. Use descriptive coordinating services for atomic cross-module work.
- Put transition and authorization preconditions in services, policies, Form Requests, model methods, database constraints, and backed enums as appropriate—not controllers, widgets, jobs, prompts, or database callbacks alone.
- Use Eloquent directly in the owning service. Introduce a small `app/Contracts` interface only for a genuinely replaceable external dependency such as medication lookup, files, auditing, clocks, or delivery.
- Provider integrations must preserve typed denial, not-found, conflict, timeout, cancellation, idempotency, and redaction behavior. An implementation that fails open violates the contract.
- Emit Laravel events only after the owning transaction commits through the transactional outbox. Consumers may notify or project state; they may not recreate clinical truth.

## Workflow

### 1. Specify the behavior before implementation

For every service action or read, record:

```text
actor + clinical context | record/current version | service action/read
...
Choose the owning models/service and database invariant together with the PostgreSQL owner. Define expected-version or lock behavior, unique/partial constraints, idempotency-key fingerprint, invariant recheck inside the transaction, audit row, and outbox event. Reads must carry actor, patient/encounter, purpose, state, and time context; an unscoped Eloquent query is not acceptable for protected clinical data.
...
### 4. Implement through the owning module

Use the Nwidart module's controllers, Form Requests, Resources, models, services, policies, jobs/events/listeners, and optional enums. Route doctor drafts through authorized Laravel/Inertia flows, patient reads through patient Inertia pages, and bytes through secure-file services. Return stable machine error codes with safe localized presentation; do not leak whether another patient's record exists.

### 5. Record clinical governance

For medication rules, red flags, controlled items, patient wording, report/referral semantics, retention, and AI clinical thresholds, record the source/provenance/version, project decision, rollback, and effective dates. Preserve clinical review where required, but use conservative configurable defaults and continue without legal approval.

## Verification

- **Unit:** transition tables, contextual access policy, version/amendment rules, medication-service behavior, conflict classification, time boundaries, redaction, and every forbidden transition.
- **Integration:** real PostgreSQL constraints/transactions/locks, grant create/revoke atomicity, append-only history, idempotency mismatch, outbox commit, file-reference eligibility, and server-owned draft conflict states.
...
- Do not claim clinical validation, statutory compliance, regulatory approval, or legal sufficiency from code/tests alone.
